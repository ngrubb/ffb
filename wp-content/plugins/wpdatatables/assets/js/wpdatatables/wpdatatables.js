...
(function ($) {
    $(function () {
        ...
        $('table.wpDataTable').each(function () {
            ...
            if (tableDescription.editable) {
                 ...
                 * Apply pickatime
                 */
                var timeFormat = tableDescription.timeFormat;
                var timepickRange = parseInt( tableDescription.timepickRange ) ? parseInt( tableDescription.timepickRange ) : 5;
                var timePickerInit = function( selector, additional_params, state ){
                ...
                timePickerInit(tableDescription.selector + '_edit_dialog input.timepicker');

                /**
                 * Apply datetimepicker
                 */
                var dateTimePickerInit = function( selector, additional_params, state ){

                    var timepickRange = parseInt( tableDescription.timepickRange ) ? parseInt( tableDescription.timepickRange ) : 5;
                    var $input = $(selector);
                    var dtPickersHtml = '<div style="display: none" class="dtpick_block">';
                    dtPickersHtml += '<input type="text" class="dt_datepick" />';
                    dtPickersHtml += '<input type="text" class="dt_timepick" />';
                    dtPickersHtml += '</div>';
    ...
    $('button.wdtClearFilters').click(function (e) {
        ...
        $('.wdtFilterDialog input:checkbox').removeAttr('checked').iCheck('update');
